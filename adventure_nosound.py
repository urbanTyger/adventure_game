...
def build_monster():
    ...
    monster.append("soundplaceholder")  # 3 sound of hitting
    for x in range(len(monsters)):
        if monster[0].lower() == monsters[x].lower():
            # 4 match final attack with chosen boss monster
            monster.append(m_final_attack[x])
        else:
            x += 1
...
def text(lines, type):
    ...
    if type == "action":
        ...
        print(f"    \033[m\033[36m{lines}\033[m\033[36mof {hero_health[1]} ]")
    else:
        print("\033[m", end="")
...
def House(place):
    if place in areas_been:
        ...
        text(f"At the {place}, you knock on the door but hear nothing.", "action")
...
def Lake(place):
    if place in areas_been:
        text(f"Since you know nothing is around, you jump directly into the {place}.", "action")
    else:
...
def Fountain(place):
    if place in areas_been:
        ...
        text(f"At the {place}, you look and admire its craftsmanship.", "action")
        areas_been.append(place)
    check_location(place)
    choose_path()
